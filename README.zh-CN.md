...
使用 CPA-Helper 模型请求地址的客户端会先经过 Helper，再转发到 CPA；该链路负责用户与号池校验、全局入口限流和请求归属。CPA 模型端口应仅监听本机或内网，公网直连 CPA 会绕过 Helper 的这些保护。
...
- **请求归属修复**：通过号池调度事件修复未知用户和 API Key，并将同一 CPA 请求 ID 下的多渠道回退记录归到同一用户；并发场景存在歧义时保持未知，避免错记费用。
...
- **号池可见范围**：每个号池可设置仅管理员、全部用户或指定用户可见；API Key 选择和渠道状态会使用同一套权限范围。
- **模型入口背压**：模型代理支持全局并发上限、短等待队列和快速 429；大请求体自动落盘，SSE 主动 Flush 并禁用 Nginx 响应缓冲。
- **CLIProxyAPI / CPAMC 集成**：配置服务地址、管理密钥、usage 队列和本地采集参数，将远端 usage 事件落入本地 SQLite。
- **号池插件监控**：跨已配置 CPA 目标查看调度选择、候选账号、最终认证 ID、请求完成状态和失败原因。
